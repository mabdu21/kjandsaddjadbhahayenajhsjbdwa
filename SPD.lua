...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("DYHUB â€¢ Session: %s â€¢ %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "DYHUB â€¢ " .. os.date("%x %X") }
        }}
    }
            ...
            if url ~= "" and url:find("discord.com/api/webhooks/") then
                local payload = {
                    ["embeds"] = {{
                        ...
                        ["footer"] = { ["text"] = "DYHUB | Sailor Piece" }
                    }}
                }
                task.spawn(function()
                    pcall(function()
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().DYHUB_Running then break end
...
task.spawn(function()
    while getgenv().DYHUB_Running do
...
ThemeManager:SetFolder("DYHUB | Sailor Piece")
SaveManager:SetFolder("DYHUB/SailorPiece")
