...
-- Новые переменные
local autoXmasOpenActive = false
local xmasOpenTask = nil
local autoCollectPresent = false
local presentName = "The Present" -- Только The Present
local rotate90Mode = false
local itemNotifyEnabled = false -- Для уведомлений о Present
local followMode = false -- Режим следования за движущимися NPC
getgenv().AttackSpeed = 0.05 -- Скорость атаки

-- Переменные для Auto God Mode
local autoGodModeEnabled = false
local godModeThreshold = 10 -- Порог здоровья для авто-респавна
local infinityYieldLoaded = false
local godModeTask = nil
local godModeCheckTask = nil

-- Языковые настройки
local currentLanguage = "English" -- Russian, English, Portuguese
local notifications = {
    Russian = {
...
-- Функция настройки ProximityPrompt
...
-- Функция поиска NPC с Flush Proximity
...
-- Функции телепортации с поддержкой поворота
...
-- Функция создания опорной части
local supportPart
...
-- Функция предсказания движения NPC
local function predictNPCMovement(npc, timeAhead)
    ...
    -- Если у NPC есть Humanoid с движением, пытаемся предсказать
    local humanoid = npc:FindFirstChildOfClass("Humanoid")
    if humanoid and humanoid.MoveDirection and humanoid.MoveDirection.Magnitude > 0 then
        -- Предсказываем позицию через timeAhead секунд
...
-- Обновленная функция calculatePosition с предсказанием движения
local spinAngle = 0
local function calculatePosition(npc)
    if not npc or not npc:FindFirstChild("HumanoidRootPart") then return CFrame.new() end
    local hrp = npc.HumanoidRootPart
    local dist = getgenv().DistanceValue or 2
    
    -- Предсказываем движение на 0.3 секунды вперед
    local predictedPosition = predictNPCMovement(npc, 0.3)
    local position
    ...
    -- Если включен режим поворота на 90 градусов
    if rotate90Mode then
...
-- Функция атаки NPC без ProximityPrompt (с отслеживанием движения)
local function attackHumanoidNoProximity(npc)
    ...
    while humanoid.Health>0 and autoFarmActive do
        -- Получаем актуальную позицию NPC каждый кадр
        if npc and npc:FindFirstChild("HumanoidRootPart") then
            local targetCFrame = calculatePosition(npc)
            
            if followMode then
                -- В режиме следования мгновенная телепортация
                instantTeleportTo(targetCFrame)
            else
...
-- Улучшенная функция AutoFarm с отслеживанием движения NPC
...
-- Ультра быстрый Xmas Open функция
local function startAutoXmasOpen()
    if xmasOpenTask then return end
    
    xmasOpenTask = task.spawn(function()
        while autoXmasOpenActive do
            pcall(function()
                -- Очень быстро открываем ВСЕ подарки
                for i = 1, 100 do -- 100 раз за цикл
                    if not autoXmasOpenActive then break end
                    
                    game:GetService("ReplicatedStorage"):WaitForChild("GachaCapsule"):FireServer()
                    
                    -- Отправляем уведомление каждые 10 открытий
                    if i % 10 == 0 then
                        game:GetService("StarterGui"):SetCore("SendNotification", {
                            Title = "DYHUB",
                            Text = getNotification("xmasOpened") .. " (" .. i .. ")",
                            Duration = 1,
                            Icon = "rbxassetid://104487529937663"
                        })
                        print("[DYHUB] " .. getNotification("xmasOpened") .. " " .. i .. " times")
                    end
                    
                    task.wait(0.001) -- Очень маленькая задержка для максимальной скорости
                end
            end)
            task.wait(0.01) -- Короткая пауза между циклами
        end
        xmasOpenTask = nil
    end)
end

-- Функция уведомления о предмете (мультиязычная)
local function itemNotify(obj)
    ...
    if hrp and obj then
        ...
        if obj:IsA("Model") and obj.PrimaryPart then
            distance = (hrp.Position - obj.PrimaryPart.Position).Magnitude
        else
            distance = (hrp.Position - obj.Position).Magnitude
        end
        
        local message = getNotification("presentFound") .. "\n" .. 
                       string.format("Distance: %.0f studs", distance)
        ...
        print("[DYHUB] " .. getNotification("presentFound") .. " at " .. math.floor(distance) .. " studs")
    end
end

-- Улучшенная функция быстрого подбора подарков (только The Present)
local function collectPresentFast(obj)
    local prompt = obj:FindFirstChildOfClass("ProximityPrompt")
    if not prompt then 
        print("[DYHUB] No ProximityPrompt found on " .. obj.Name)
        return 
    end
    ...
    print("[DYHUB] " .. getNotification("presentCollecting"))
    
    -- Устанавливаем мгновенное срабатывание
    prompt.HoldDuration = 0
    
    -- Телепортация к предмету
    local targetPos
    if obj:IsA("Model") and obj.PrimaryPart then
        targetPos = obj.PrimaryPart.Position + Vector3.new(0, 3, 0)
    else
        targetPos = obj.Position + Vector3.new(0, 3, 0)
    end
    
    -- Мгновенная телепортация
    instantTeleportTo(CFrame.new(targetPos))
    
    -- ОЧЕНЬ быстрое собирание (500 раз в секунду)
    for i = 1, 500 do
        if not autoCollectPresent then break end
        ...
        pcall(function()
            ...
            -- Дополнительные клики для надежности
            prompt:InputHoldBegin()
            task.wait(0.0001) -- Очень маленькая задержка
            prompt:InputHoldEnd()
        end)
        
        task.wait(0.002) -- 500 раз в секунду (0.002 сек)
    end
    
    print("[DYHUB] Finished collecting " .. obj.Name)
end

-- Функция авто-спама The Present
local function startPresentSpam()
    task.spawn(function()
        while autoCollectPresent do
            pcall(function()
                -- Ищем все The Present на карте
                local presents = {}
                for _, obj in pairs(workspace:GetDescendants()) do
                    if (obj:IsA("Model") or obj:IsA("Part")) then
                        if obj.Name == "The Present" then
                            table.insert(presents, obj)
                        end
                    end
                end
                ...
                if hrp and #presents > 0 then
                    table.sort(presents, function(a, b)
                        local distA = (hrp.Position - (a:IsA("Model") and a.PrimaryPart and a.PrimaryPart.Position or a.Position)).Magnitude
                        local distB = (hrp.Position - (b:IsA("Model") and b.PrimaryPart and b.PrimaryPart.Position or b.Position)).Magnitude
                        return distA < distB
                    end)
                    
                    -- Быстро подбираем ближайший
                    for _, present in ipairs(presents) do
                        if autoCollectPresent then
                            -- Уведомление
                            if itemNotifyEnabled then
                                itemNotify(present)
                            end
                            
                            collectPresentFast(present)
                            task.wait(0.05) -- Маленькая пауза между предметами
                        else
                            break
                        end
                    end
                else
                    -- Если предметов нет, ждем и ищем снова
                    task.wait(0.5)
                end
            end)
            task.wait(0.1) -- Быстрая проверка
        end
    end)
end

-- Функция загрузки Infinity Yield
...
-- Улучшенная функция авто-респавна при низком здоровье
                ...
                if character then
                    ...
                    if humanoid and humanoid.Health > 0 then
                        ...
                        if humanoid.Health <= godModeThreshold then
                            ...
                            -- Загружаем Infinity Yield если еще не загружен
                            loadInfinityYield()
                            
                            -- Используем команду ;reset из Infinity Yield
                            print("[DYHUB] " .. getNotification("resetCommand"))
                            game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(";reset", "All")
                            
                            -- Дополнительный вызов через RemoteFunction если есть
                            if ReplicatedStorage:FindFirstChild("ResetCharacter") then
                                ReplicatedStorage:FindFirstChild("ResetCharacter"):InvokeServer()
                            end
                            
                            -- Ждем 2 секунды после респавна
                            task.wait(2)
                        end
                    end
                end
...
-- Переподключение при появлении персонажа
LocalPlayer.CharacterAdded:Connect(function()
    ...
    if autoXmasOpenActive then startAutoXmasOpen() end
    if autoCollectPresent then startPresentSpam() end
...
-- Добавление выбора языка в MiscTab
...
MainTab:Toggle({
    ...
    Callback = function(value)
        ...
        if value then
            -- Пытаемся загрузить Infinity Yield при включении
            loadInfinityYield()
            startAutoGodMode()
...
MainTab:Toggle({
    Title = "Follow Mode (Track Moving NPC)",
    Default = false,
    Callback = function(value)
        followMode = value
    end
...
-- Авто Xmas Open в MainTab
MainTab:Toggle({
    Title = "Auto Xmas Open (ULTRA FAST)",
    Default = false,
    Callback = function(value)
        autoXmasOpenActive = value
        if value then
            startAutoXmasOpen()
        elseif xmasOpenTask then
            task.cancel(xmasOpenTask)
            xmasOpenTask = nil
        end
    end
})
...
-- Включение отображения имени в ESP
EspTab:Toggle({
    Title = "ESP Name",
    Default = true,
    Callback = function(value)
...
-- Включение отображения здоровья в ESP
EspTab:Toggle({
    Title = "ESP Health",
    Default = true,
    Callback = function(value)
...
-- Включение отображения дистанции в ESP
EspTab:Toggle({
    Title = "ESP Distance",
    Default = false,
    Callback = function(value)
...
-- Функция применения Hitbox к NPC
...
-- Сканирование NPC (мультиязычное)
...
-- Misc Tab Переменные для эффектов освещения
...
local function itemNotifyGeneral(obj)
    local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if hrp and obj then
        local distance
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "DYHUB",
            Text = getNotification("itemFound") .. ": " .. obj.Name .. "\n" .. 
                   string.format("Distance: %.0f studs", distance),
            Duration = 3,
            Icon = "rbxassetid://104487529937663"
        })
        
        print("[DYHUB] " .. getNotification("itemFound") .. " " .. obj.Name .. " at " .. math.floor(distance) .. " studs")
    end
end
...
-- Уведомление для The Present
CollectTab:Toggle({
    Title = "The Present Notify",
    Default = false,
    Callback = function(enabled)
...
-- Авто-спам The Present
CollectTab:Toggle({
    Title = "Auto Collect The Present (ULTRA FAST)",
    Default = false,
    Callback = function(value)
        autoCollectPresent = value
        if value then
            startPresentSpam()
        end
    end
...
-- Автоматическое получение названий предметов из Gears
local shop = {}
...
-- Создание dropdown для выбора предметов из магазина
ShopTab:Dropdown({ 
...
-- Создание dropdown для выбора количества покупок
ShopTab:Dropdown({ 
...
-- Переключатель включения/выключения авто-покупки
ShopTab:Toggle({
    Title = "Auto Buy", 
    Default = false,
    Callback = function(enabled)
...
-- Переключатель для авто-гача персонажей
ShopTab:Toggle({
...
-- Переключатель для авто-гача скинов
ShopTab:Toggle({
...
-- Определение функции Request, имитирующей ui.Creator.Request
ui.Creator.Request = function(requestData)
    local HttpService = game:GetService("HttpService")
    
    -- Попытка различных HTTP методов
    local success, result = pcall(function()
        if HttpService.RequestAsync then
            -- Метод 1: Использовать RequestAsync, если доступно
            ...
            -- Метод 2: Резервный вариант GetAsync
...
-- Добавление опции "All" для Dropdown
local dropdownValues = {"All"}
for _, v in ipairs(skillList) do
    table.insert(dropdownValues, v)
end

-- Dropdown для выбора скиллов для авто-использования
SkillTab:Dropdown({
    Title = "Set Auto Skill",
    Values = dropdownValues,
    Multi = true,
    Callback = function(values)
        -- Если выбрано All, использовать все скиллы
...
-- Переключатель для включения/выключения авто-скиллов
SkillTab:Toggle({
    Title = "Auto Skill",
    Default = false,
    Callback = function(enabled)
