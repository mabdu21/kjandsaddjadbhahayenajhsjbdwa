...
-- Full updated aimbot script (replace your current script with this or merge as needed)
MainTab:Section({ Title = "TEST Aimbot", Icon = "target" })
...
local Settings = {
    Aimbot = {
        ...
        -- NEW
        TWallUI = false,           -- Tough wall toggle (ล็อคทะลุกำแพง)
        Smooth = 12,               -- smoothing speed (higher => faster snap)
        MinHealth = 20,            -- don't aim at players with health < MinHealth
    }
}
...
local aimSmooth = Settings.Aimbot.Smooth or 12
local minHealth = Settings.Aimbot.MinHealth or 20

-- store camera type when locking so we can restore later
local originalCameraType = nil
local isCameraForced = false
...
local function CreateMobileButton()
    ...
    mobileButton.MouseButton1Click:Connect(function()
        ...
        Settings.Aimbot.Enable = AimbotEnabled
        if not AimbotEnabled then
            -- unlock target and restore camera
            LockedTarget = nil
            if isCameraForced and Workspace.CurrentCamera then
                Workspace.CurrentCamera.CameraType = originalCameraType or Workspace.CurrentCamera.CameraType
                isCameraForced = false
            end
        else
            task.spawn(FindNearestTarget)
        end
...
-- NEW: Min health input so aimbot will skip low-health players
MainTab:Input({
    Title = "Min Target Health (Skip if < value)",
    Default = tostring(Settings.Aimbot.MinHealth),
    Placeholder = "Ex: 20",
    Callback = function(text)
        local num = tonumber(text)
        if num and num >= 0 then
            minHealth = num
            Settings.Aimbot.MinHealth = num
            print("[Aimbot] MinHealth set to", num)
        else
            warn("[Aimbot] Invalid MinHealth value")
        end
    end
})
...
-- Smoothness value input (NEW)
MainTab:Input({
    Title = "Aim Smoothness (Higher = snappier)",
    Default = tostring(Settings.Aimbot.Smooth),
    Placeholder = "Ex: 12",
    Callback = function(text)
        local num = tonumber(text)
        if num and num > 0 then
            aimSmooth = num
            Settings.Aimbot.Smooth = num
        else
            warn("[Aimbot] Invalid smooth value")
        end
    end
})
        ...
        function(name, inputState, inputObject)
            if inputState == Enum.UserInputState.Begin and AimbotEnabled then
                if LockedTarget then
                    -- Already locked → unlock
                    LockedTarget = nil
                    -- restore camera type when unlocking
                    if isCameraForced and Workspace.CurrentCamera then
                        Workspace.CurrentCamera.CameraType = originalCameraType or Workspace.CurrentCamera.CameraType
                        isCameraForced = false
                    end
                    ...
                    if target then
                        ...
                        -- Force camera type to Scriptable while locked so player view stays aimed
                        local cam = Workspace.CurrentCamera
                        if cam then
...
----------------------------------------------------------
-- AIMBOT CORE (1st & 3rd Person Support) - IMPROVED
----------------------------------------------------------
local Camera = Workspace.CurrentCamera
...
local function GetCameraPosition()
    if Camera and Camera.CameraType == Enum.CameraType.Scriptable then
        ...
        local head = LocalPlayer.Character and (LocalPlayer.Character:FindFirstChild("Head") or LocalPlayer.Character:FindFirstChild("HumanoidRootPart"))
        return head and head.Position or (Camera and Camera.CFrame.Position or Vector3.new())
    end
end
...
local function IsValidTarget(plr)
    ...
    -- NEW: skip if target health is below configured minimum
    minHealth = Settings.Aimbot.MinHealth or minHealth
    if char.Humanoid.Health < minHealth then
        return false
    end
    ...
    -- Wall check: skip when ToughWallEnabled == true
...
-- Main Aimbot Loop (improved smoothing + camera lock handling)
RunService.RenderStepped:Connect(function(dt) -- dt available for smooth lerp
    if not AimbotEnabled then
        -- if disabled, make sure we don't keep forced camera
        if isCameraForced and Workspace.CurrentCamera then
            Workspace.CurrentCamera.CameraType = originalCameraType or Workspace.CurrentCamera.CameraType
            isCameraForced = false
        end
    ...
    local target = LockedTarget
    local valid, targetPartRef
    if target then
        valid, targetPartRef = IsValidTarget(target)
    end
    if (not target) or not valid then
        ...
        if not target then
            ...
            -- if we auto-locked (not via keybind) force camera type so view goes to target
            if not isCameraForced and cam then
                originalCameraType = cam.CameraType
                cam.CameraType = Enum.CameraType.Scriptable
                isCameraForced = true
            end
    ...
    if target and target.Character and target.Character:FindFirstChild(LockPart) then
        ...
        -- Smooth lookAt: lerp camera orientation towards targetPos while keeping camera position
        -- For 1st/3rd person this will feel smooth and consistent.
        local currentPos = cam.CFrame.Position
        local desiredCFrame = CFrame.new(currentPos, targetPos)

        -- aimSmooth controls how fast we lerp (higher = faster)
        local alpha = math.clamp(aimSmooth * dt, 0, 1)
...
-- เพิ่มค่าใน Settings สำหรับบันทึกตำแหน่งล่าสุด
Settings.Aimbot.DragUI = false
Settings.Aimbot.MobileButtonPosition = Settings.Aimbot.MobileButtonPosition or UDim2.new(1, -40, 1, -40)
