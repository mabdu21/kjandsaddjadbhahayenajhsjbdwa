...
-- Disable auto features to save memory
AutoSaveEnabled = false
AutoLoadEnabled = false

-- Now run the main script

if getgenv().sudadlolExecuted then
    return
end
getgenv().sudadlolExecuted = true
...
pcall(function() Window:Tag({ Title = "PAID", Color = Color3.fromHex("#db7093") }) end)
...
Tabs.Misc:Button({
    ...
    Callback = function()
        ...
        for _, textchannel in ipairs(textchannels) do
            ...
            if murd then
                local message = string.format("%s Is Murderer | Script Used: DYHUB", murd.Name)
                textchannel:SendAsync(message)
            else
                local message = "No Murderer Found | Script Used: DYHUB"
                textchannel:SendAsync(message)
            end
        end
    end
...
Tabs.Misc:Button({
    ...
    Callback = function()
        ...
        for _, textchannel in ipairs(textchannels) do
            ...
            local hero = findHero()
            if sher then 
                local message = string.format("%s Is Sheriff | Script Used: DYHUB", sher.Name)
                textchannel:SendAsync(message)
            elseif hero then
                local message = string.format("%s Is Hero | Script Used: DYHUB", hero.Name)
                textchannel:SendAsync(message)
            else
                local message = "No Sheriff/Hero Found | Script Used: DYHUB"
                textchannel:SendAsync(message)
            end
        end
    end
